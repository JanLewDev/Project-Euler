...
struct BigNum {
	...
	void operator=(ll n) {
		sign = 1;
		if (n < 0) sign = -1, n = -n;
		for (; n > 0; n /= base) digits.push_back((int)(n % base));
	}

	void operator=(const string& num) { parse_string(num); }
	...
	BigNum operator+(BigNum const& d) const {
		if (sign == d.sign) {
			BigNum res = d;
			int carry = 0;
			for (int i = 0; i < max(size(digits), size(d.digits)) || carry;
				 i++) {
				if (i == size(res.digits)) res.digits.push_back(0);
	...
	BigNum operator+(ll n) const { return *this + BigNum(n); }
		...
		if (sign == d.sign) {
			if (abs() >= d.abs()) {
				...
				for (int i = 0; i < max(size(digits), size(d.digits)) || carry;
					 i++) {
					res.digits[i] -=
						carry + (i < size(d.digits) ? d.digits[i] : 0);
	...
	void operator*=(int n) {
		...
		for (int i = 0; i < size(digits) || carry; i++) {
			if (i == size(digits)) digits.push_back(0);
	...
	void operator+=(BigNum const& d) { *this = *this + d; }
	...
	bool operator<(BigNum const& d) const {
		...
		if (size(digits) != size(d.digits))
			return size(digits) * sign < size(d.digits) * d.sign;
	...
	bool operator>(BigNum const& d) const { return d < *this; }

	bool operator<=(BigNum const& d) const { return !(d < *this); }

	bool operator>=(BigNum const& d) const { return !(*this < d); }
