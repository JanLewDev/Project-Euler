...
// PrimePi function from
// https://am-just-a-nobody.blogspot.com/2015/11/c-code-for-primepi-function.html
...
ull PrimePi(ull n) {
	...
	for (ull p = 2; p <= v; p++) {
		...
		for (ull i = q; i <= end; i += p * j) Used[i] = 1;

		for (ull i = v; i >= q; i--) Lower[i] -= Lower[i / p] - temp;
	}
	return res;
}
...
ll eightDivisors(ll n) {
	...
	for (auto i : primes) {
		if (fastpow(i, 3) <= n) {
			...
			if (deficit > i) add--;
			ans += add;
			// cout << i << " " << deficit << " " << PrimePi(deficit) << " " <<
			// ans << "\n";
		} else
			break;
	}
	...
	for (int i = 0; i < size(primes); i++) {
		...
		for (int j = i + 1; j < size(primes); j++) {
			...
			if (add > 0)
				...
				break;
			// cout << primes[i] << " " << primes[j] << " " << deficit << " " <<
			// PrimePi(deficit) << " " << ans << "\n";
		}
	}
